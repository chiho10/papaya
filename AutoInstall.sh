...
acl SSH dst 49.213.16.18/32 # change your own ip server
...
wget -O /etc/openvpn/easy-rsa/vars "https://github.com/chiho10/banana/raw/master/vars"
...
ip=$(ifconfig | awk -F':' '/inet addr/&&!/127.0.0.1/&&!/127.0.0.2/{split($2,_," ");print _[1]}')
cat > /etc/openvpn/chiho10.ovpn <<-END
# OpenVPN Configuration CHIHO10 Server
client
dev tun
proto tcp
#for tcp 1194
...
#remote $ip 1195
#change port and proto as you want
#there the prosedur edit type connection
#proto udp #with port active udp 25 & 110 choose as u want
#change 1194 to 25 or 110 as the port udp u want to use
...
verb 3
END
echo '<ca>' >> /etc/openvpn/chiho10.ovpn
cat /etc/openvpn/ca.crt >> /etc/openvpn/chiho10.ovpn
echo '</ca>' >> /etc/openvpn/chiho10.ovpn
sed -i $ip /etc/openvpn/chiho10.ovpn
...
state=SG
locality=SINGAPORE
organization=CHIHO10
organizationalunit=PROVIDER
commanname=CHIHO10.NET
email=ADMIN@CHIHO10.NET
...
clear
echo "-------- Information --------"
echo "Install Stunnel4 Successfuly"
echo "-----------------------------"
echo "OpenSSH           : 22"
